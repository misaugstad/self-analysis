...
```{r plotting.odds.ratios, fig.height=3, fig.width=4, echo=FALSE}
...
ggplot(df, aes(x = odds, y = variable)) +
  geom_vline(aes(xintercept = 1), size = .25, linetype = "dashed") +
  geom_errorbarh(aes(xmax = ub, xmin = lb), size = .5, height = .2, color = "gray50") +
  geom_point(size = 3.5, color = "orange") +
  theme_bw() +
  theme(panel.grid.minor = element_blank()) +
  # scale_y_continuous(breaks = nrow(df):1, labels = df$variable) +
  # scale_x_continuous(breaks = seq(0,7,1) ) +
  # coord_trans(x = "log10") +
  # scale_x_log10(labels = function(x) { format(x, digits = 3, scientific = FALSE)}) +
  scale_x_log10() +
  ylab("") +
  xlab("Odds ratio; <=2 (log scale)")
  # facet_grid(. ~ productivity.rating)
