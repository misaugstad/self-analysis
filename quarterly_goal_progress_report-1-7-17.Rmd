...
#### Subsetting Data
```{r subset_data, echo=TRUE, cache=TRUE}
...
date.to.use <- min(end.date, current.date)
weeks.passed <- (as.numeric(date.to.use - start.date) + 1) / 7
...
if(!fruit.on.track)
{
  (expected.fruit.freq * 13.0 - fruit.count) / (13.0 - weeks.passed)
}
...
{
  (expected.workout.freq * 13.0 - workout.count) / (13.0 - weeks.passed)
}
...
if(!journal.on.track)
{
  (expected.journal.freq * 13.0 - journal.count) / (13.0 - weeks.passed)
}
```